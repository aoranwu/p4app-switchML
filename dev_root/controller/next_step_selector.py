...
class NextStepSelector(Control):
    ...
    def __init__(self, target, gc, bfrt_info, use_multipipe=False, use_model=False):
        # Set up base class
        ...
        if not use_multipipe:
            self.add_default_entries(use_model)
        else:
            self.add_default_entries_for_pipes([0,1,2],use_model)

    def _clear(self):
        super(NextStepSelector, self)._clear()
        self.reset_counters()

    def add_default_entries(self, use_model=False):
        ''' Add default entries '''
        if not use_model:
    ...
    def add_default_entries_for_pipes(self, pipes ,use_model=False):
        for pipe in pipes:
            self.add_default_entries_for_pipe(pipe, use_model)

    def add_default_entries_for_pipe(self, pipe,use_model=False):
        ''' Add default entries '''
        if not use_model:
        # Special recirculation ports used for harvest passes
            port = {1: 452, 2: 324, 3: 448, 4: 196, 5: 192, 6: 68, 7: 64}
        ...
        entries = [
            ...
            # 256B packets: Pipe 0 only at the moment
            # Last packet -> recirculate for harvest
            ( PacketSize.MTU_256, None, PacketType.CONSUME0,  Flag.LAST, None, None, 1, 'recirculate_for_HARVEST7', recirc_ports[pipe]),
            ## Just consume a CONSUME packet if it is not the last and we haven't seen it before
            ( PacketSize.MTU_256, None, PacketType.CONSUME0,       None,    0, None, 2, 'finish_consume', None),
            ## CONSUME packets that are retransmitted packets to a full slot -> recirculate for harvest
            ( PacketSize.MTU_256, None, PacketType.CONSUME0, Flag.FIRST, None, None, 3, 'recirculate_for_HARVEST7', recirc_ports[pipe]),
            ## Drop others
            ( PacketSize.MTU_256, None, PacketType.CONSUME0,       None, None, None, 4, 'drop', None),
        ...
        for e in entries:
            success, error_msg = self.add_entry_for_pipe(
                **{
                    'packet_size': e[0],
                    'worker_id': e[1],
                    'packet_type': e[2],
    ...
    def add_entry_for_pipe(self,
        ...
        # Add entry
        self.table.entry_add(self.targets[pipe], [
            self.table.make_key([
                self.gc.KeyTuple('ig_md.switchml_md.packet_size',
                                 packet_size[0], packet_size[1]),
                self.gc.KeyTuple('ig_md.switchml_md.worker_id', worker_id[0],
