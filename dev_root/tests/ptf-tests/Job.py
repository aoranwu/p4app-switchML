...
from SetSwitchType import SetSwitchType
# from SetUpwardPort import SetUpwardPort
from GetPortFromWorkerID import GetPortFromWorkerID
...
class Job(Cmd, object):
    ...
    def run(self):
        with futures.ThreadPoolExecutor(max_workers=1) as executor:
            # start CLI
            command_loop_future = executor.submit(self.cmdloop)

            #self.mac_learning.poll()

            # wait for command loop to end
            command_loop_future.result()
    ...
    def do_worker_add_udp(self, arg):
        'Add a UDP SwitchML worker. Usage: worker_add_udp <worker rank> <total number of workers> <MAC address> <IP address> [<UDP port>]'
        try:
            result = arg.split()
            ...
            if len(result)>4:
                ...
                self.worker_add_udp(rank, count, mac, ip,udp_port)
            else:
    ...
    # AMEDEO
    def do_clear_pkt_counters(self, arg):
        #self.logger.info("Clearing pkt counters...")

        # this should work, but it doesn't.
        #self.consume_counter.entry_del(self.target)
        #self.harvest_counter.entry_del(self.target)
        self.pkt_counter.entry_del(self.target)

        count = self.pkt_counter.info.size
        #self.logger.info("Clearing {} keys for counter table {}".format(count,
        #                                                   self.pkt_counter.info.name_get()))
        self.pkt_counter.entry_mod(
            self.target,
            [self.pkt_counter.make_key([gc.KeyTuple('$COUNTER_INDEX', i)])
             for i in range(count)],
            [self.pkt_counter.make_data(
    ...
    def do_set_non_root_switch(self, arg):
        # set_non_root_switch <upward_port> <upper_switch_mac> <upper_switch_ip> [upper_switch_udp_port]
        ...
        self.set_switch_type.set_default_entry(0,upward_port)
        self.set_dst_addr.add_udp_entry(0xffff,arg.split()[1],arg.split()[2])
        if len(arg.split())>=4:
        # set udp port for the worker
            self.set_dst_addr.set_udp_port_for_worker(0xffff,int(arg.split()[3]))
        else:
            self.set_dst_addr.set_udp_port_for_worker(0xffff,udp_port=0xbeef)


    # Print
    def do_pkt_counters(self, arg):

        self.pkt_counter.operations_execute(self.target, 'Sync')
        count = self.pkt_counter.info.size

        resp = self.pkt_counter.entry_get(
                          self.target,
                          [self.pkt_counter.make_key([gc.KeyTuple('$COUNTER_INDEX', i)])
                           for i in range(count)],
                          flags={"from_hw": False})

        opcode_dict = {"0000": "SND_FIRST",
                       "0001": "SND_MIDDLE",
                       "0010": "SND_LAST",
                       "0011": "SND_LAST_IMM",
                       "0100": "SND_ONLY",
        ...
        for v, k in resp:
            ...
            bitstring = bin(idx)[2:].zfill(8)

            pkts_value = v['$COUNTER_SPEC_PKTS']
            bytes_value = v['$COUNTER_SPEC_BYTES']

            if pkts_value != 0 or bytes_value != 0:
                if bitstring[4:8] in opcode_dict:
                    opcode = opcode_dict[bitstring[4:8]]
                else:
                    opcode = "NONE"

                if bitstring[0:4] in pkt_type_dict:
                    pkt_type = pkt_type_dict[bitstring[0:4]]

                    if pkt_type not in values:
                        values[pkt_type] = {}
                    values[pkt_type][opcode] = (pkts_value, bytes_value)
                    nonzero_opcodes.add(opcode)

        header = " "*20
        for op in nonzero_opcodes:
            header += "{:12} ".format(op)
        print (header)
        print ("_"*len(header))

        for k,v in values.items():
            line1 = "{:10} (Pkts) | ".format(k)
            line2 = "{:10} (B)    | ".format(k)

            for op in nonzero_opcodes:
                if op in v:
                    line1 += "{:12} ".format(v[op][0])
                    line2 += "{:12} ".format(v[op][1])
                else:
                    line1 += "{:12} ".format(0)
                    line2 += "{:12} ".format(0)
    ...
    def mac_address_clear_all(self):
        self.non_switchml_forward.worker_clear_all()
    ...
    def worker_add_udp(self,
                       ...
                       worker_mac, worker_ip, udp_port=0xbee0):
        ...
        # set udp port for the worker
        self.set_dst_addr.set_udp_port_for_worker(worker_rank,udp_port)

        # add to worker_id egress port mapping
        dev_port = self.ports.get_dev_port(port, lane)
        self.get_port_from_worker_id.set_port_for_worker_id(worker_rank, dev_port)
    ...
    def __init__(self, gc, bfrt_info,
        ...
        self.get_port_from_worker_id = GetPortFromWorkerID(self.gc, self.bfrt_info)
        ...
        self.set_switch_type = SetSwitchType(self.gc, self.bfrt_info)
        # self.set_upward_port = SetUpwardPort(self.gc, self.bfrt_info)
        ...
        if self.workers:
            for worker in self.workers:
                ...
                self.mac_address_add(worker.mac.upper(), worker.front_panel_port, worker.lane)
        elif ports_file:
        ...
        self.pkt_counter = self.bfrt_info.table_get("pipe.Ingress.pkt_counter")
        ...
        self.grpc_future = self.grpc_executor.submit(self.grpc_server.run, self.event_loop, self)
