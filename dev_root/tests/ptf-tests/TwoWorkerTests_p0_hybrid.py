...
class TwoWorkerTest(SwitchMLTest):
    ...
    def setUp(self):
        ...
        self.switch_mac           = "06:00:00:00:00:02"
        self.switch_ip            = "198.19.200.202"
        ...
        self.all_workers = [
        Worker(mac="de:08:be:d7:01:47", ip="198.19.200.50", udp_port=12345, front_panel_port=1, lane=0, speed=100, fec='none'),
        Worker(mac="62:68:1b:cb:18:05", ip="198.19.200.49", udp_port=12345, front_panel_port=2, lane=0, speed=100, fec='none')
        ]
        ...
        ((self.pktW0S0, self.expected_pktW0S0),
         (self.pktW1S0, self.expected_pktW1S0)) = self.make_switchml_packets(self.switchml_workers,
                                                                             0x0000, 1, self.switch_udp_port,base_worker_id=2,switch_num=2)
...
class BasicReduction(TwoWorkerTest):
    ...
    def runTest(self):
        ...
        send_packet(self, 4, self.pktW1S0)
        ...
        verify_packet(self, self.expected_pktW0S0, 0, timeout=5)
        verify_packet(self, self.expected_pktW0S0, 4, timeout=5)
        # verify_packet(self, self.expected_pktW1S0, 4)
