...
class TwoWorkerTest(SwitchMLTest):
    ...
    def setUp(self):
        ...
        self.all_workers = [
        Worker(mac="ca:0c:b1:05:46:77", ip="198.19.200.50", udp_port=12345, front_panel_port=1, lane=1, speed=100, fec='none'),
        Worker(mac="02:ec:cd:ec:30:c2", ip="198.19.200.49", udp_port=12345, front_panel_port=1, lane=2, speed=100, fec='none')
        ]
        ...
        ((self.pktW0S0, self.expected_pktW0S0),
         (self.pktW1S0, self.expected_pktW1S0)) = self.make_switchml_packets(self.switchml_workers,
                                                                             0x0000, 1, self.switch_udp_port,switch_num=2)
...
class BasicReduction(TwoWorkerTest):
    ...
    def runTest(self):
        ...
        verify_packet(self, self.expected_pktW0S0, first_worker_port,timeout=5)
        verify_packet(self, self.expected_pktW1S0, second_worker_port,timeout=5)
